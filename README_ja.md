...
- **該当する revision が無ければエラーです。** tip に落とすと、設定が効いているように
  見えて何もしていない状態になります
- **lock は後退しません。** 「これより新しいものは取らない」は「戻る」ではないので、
  すでに新しい revision を指している lock はそのまま維持し、その旨を報告します。これが
  無いと、設定を有効にした瞬間にこのリポジトリ自身の案内 skill が 1.0 より前の revision
  ——存在しないコマンドを説明するもの——に巻き戻ります
